			...
			<table style="text-align: left;" width ="100%">
					<tr>
						...
						$id=$_GET["id"];
								 ...
								 $query2 =mysql_query("SELECT * FROM `temetyadministracja` WHERE id_kategoria = $id");

	 								while ($row1 = mysql_fetch_row($query2))
									{
										$i=$row1[4];
										$p=$row1[0];
										$zliczposty =mysql_query("SELECT * FROM `postyadministracja` WHERE id_temat=\"$i\" and id_kategoria= \"$p\"");
										$zliczpart1 =mysql_num_rows($zliczposty);	

											echo "<tr>";
											echo "<td class=\"xd\"><a href = \"posty3.php?id=$i&amp;id_kategoria=$p \">".$row1[1]. "</a><br>";
											echo "$row1[2]</td>"; //opis
							...
							<table>
								...
								<?php
									...
									if(isset($_POST['dodaj']))
									{
										...
										$zmienna=mysql_query("INSERT INTO temetadministracja(nazwa,opis,id_kategoria) VALUES (\"$nazwa\",\"$tresc\",\"$idkat\")");
