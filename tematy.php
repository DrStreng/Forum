...
<div class="tablewew">
	<table style="text-align: left;" width="100%">
		<tr><td  class="xd2" >
			...
			<table style="text-align: left;" width ="100%">
					<tr>
						<td class="xd" style="height: 80%;">Temat</td>
						...
						$id=$_GET["id"];
								 ...
								 $query2 =mysql_query("SELECT * FROM `tematyogame` WHERE id_kategoria = $id");

	 								while ($row1 = mysql_fetch_row($query2))
									{
										$i=$row1[4];
										$p=$row1[0];
										$zliczposty =mysql_query("SELECT * FROM `postyogame` WHERE id_temat=\"$i\" and id_kategoria= \"$p\"");
										$zliczpart1 =mysql_num_rows($zliczposty);	

											echo "<tr>";
											echo "<td class=\"xd\"><a href = \"posty1.php?id=$i&amp;id_kategoria=$p \">".$row1[1]. "</a><br>";
											echo "$row1[2]</td>"; //opis
							...
							<table>
								...
								<?php
									...
									if(isset($_POST['dodaj']))
									{
										...
										$zmienna=mysql_query("INSERT INTO tematyogame(nazwa,opis,id_kategoria) VALUES (\"$nazwa\",\"$tresc\",\"$idkat\")");
